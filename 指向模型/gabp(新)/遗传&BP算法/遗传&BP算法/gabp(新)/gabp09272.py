...
def getDecode(population, population0, population1, population2, population3, encodelength, decisionvariables):
    population_decimal = (
            ...
            (decisionvariables[0][1] - decisionvariables[0][0]) + 0.5 * (
                        decisionvariables[0][0] + decisionvariables[0][1]))
...
if __name__ == "__main__":
    ...
    x_train = data_x[:int(N * 0.8), :]
    y_train = data_y[:int(N * 0.8)]
    x_test = data_x[int(N * 0.8):, :]
    y_test_y = data_y[int(N * 0.8):]
    print('数据准备完成...')
    ...
    # 训练
    for i in range(300):
        Weight1, Weight2, Value1, Value2 = fitnessFunction(x_train, y_train, weight1, weight2, value1, value2, 3, 2, 1,
                                                           0)
    # 预测
