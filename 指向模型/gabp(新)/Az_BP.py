...
# 原始方位角数据 eff
# 模型预测方位数据
X_norm = scale.fit_transform(data_set)
X = tensor_tran(data_set[:, :3]).to(torch.float).reshape(data_set[:, :3].shape[0], 3)
BP_pre = BP_net(X).detach().numpy()
# BP_pre1 = scale.inverse_transform(BP_pre)
