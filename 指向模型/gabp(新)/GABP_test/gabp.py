...
BP_prediction_all = BP_net(x_train).detach().numpy()
print("经典BP训练完成...")
# ================================= #
# plt.figure()
...
gaBP_prediction_all = gaBP_net(x_train).detach().numpy()
end_time = time.time()
print("遗传优化完成...")
#
...
plt.figure()
plt.plot(BP_prediction_all, label='BP预测（训练集）', c='r')
plt.plot(y_train, label='真值', c='b')
plt.grid(ls='--')
plt.legend()

plt.figure()
plt.plot(gaBP_prediction_all, label='GABP预测（训练集）', c='r')
plt.plot(y_train, label='真值', c='b')
plt.grid(ls='--')
plt.legend()

plt.figure()
plt.plot(BP_lossList, label='BP_loss', c='r')
plt.plot(gaBP_lossList, label='GABP_loss', c='b')
plt.grid(ls='--')
# plt.ylabel("BP误差下降曲线")
plt.legend()
...
plt.figure()
gaBP_Az_error = y_test.flatten() - gaBP_prediction.flatten()
plt.plot(gaBP_Az_error, label='gaBP_Az_error')
plt.legend()

plt.figure()
BP_Az_error = y_test.flatten() - BP_prediction.flatten()
plt.plot(BP_Az_error, label='BP_Az_error')
plt.legend()
